...
data <- data.frame(read.csv(paste(file.path, "Plate1HCCJ2ZNonCSCCSC052217_data.csv", sep=""), header = TRUE, sep = ",", stringsAsFactors = FALSE))
...
#}
splitcolumns <- str_split_fixed(data$Sample, " ", 3)
data <- cbind.data.frame(data, splitcolumns)
data <- subset(data, select = -c(Sample, `1`, `2`))
colnames(data) <- c("Well", "Target", "CT", "Sample")

for(i in data){
  if(data$Well == A1 | Well == B1:B6){
    data$User[i] <- "Clay"
