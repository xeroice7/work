...
#######################################
# cBio - Lung Squamous Cell Carcinoma TCGA (Provisional) - 504 Patients
#######################################
...
patientdata <- data.frame(read.csv("~/Desktop/Clay/Mass Spec Results/WebData/Lung/lusc_tcga_prov/tcga/data_bcr_clinical_data_patient.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE))
...
expressiondata <- read.csv("~/Desktop/Clay/Mass Spec Results/WebData/Lung/lusc_tcga_prov/tcga/data_RNA_Seq_v2_expression_median.csv", check.names=FALSE, sep = ",", stringsAsFactors = FALSE)
...
meta <- factor(c("M0", "M1", "M1a", "M1b"))
meta <- ordered(meta, levels = c("M0", "M1", "M1a", "M1b"))
grades <- factor(c("T1", "T1a", "T1b", "T2", "T2a", "T2b", "T3", "T4"))
grades <- ordered(grades, levels = c("T1", "T1a", "T1b", "T2", "T2a", "T2b", "T3", "T4"))
stages <- factor(c("I", "IA", "IB", "II", "IIA", "IIB", "III", "IIIA", "IIIB", "IV"))
stages <- ordered(stages, levels = c("I", "IA", "IB", "II", "IIA", "IIB", "III", "IIIA", "IIIB", "IV"))
