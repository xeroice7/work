...
patientdata <- data.frame(read.csv("~/Desktop/Clay/Mass Spec Results/WebData/Ovary/ov_tcga/data_bcr_clinical_data_patient.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE))

colnames(patientdata) <- lapply(patientdata[4,], as.character)

patientdata <- patientdata[-(1:4),]

patientdata <- subset(patientdata, select = c(PATIENT_ID, GRADE, CLINICAL_STAGE, OS_STATUS)) 
...
expressiondata <- read.csv("~/Desktop/Clay/Mass Spec Results/WebData/Ovary/ov_tcga/data_expression_median_edited.csv", sep = ",", stringsAsFactors = FALSE, check.names = FALSE)
...
spec.stages <- factor(c("IA", "IB", "IC", "IIA", "IIB", "IIC", "IIIA", "IIIB", "IIIC", "IV"))
spec.stages <- ordered(spec.stages, levels = c("IA", "IB", "IC", "IIA", "IIB", "IIC", "IIIA", "IIIB", "IIIC", "IV"))
