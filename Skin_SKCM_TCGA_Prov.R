...
patientdata <- data.frame(read.csv("~/Desktop/Clay/Mass Spec Results/WebData/Skin/skcm/tcga/data_bcr_clinical_data_patient.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE))
...
expressiondata <- read.csv("~/Desktop/Clay/Mass Spec Results/WebData/Skin/skcm/tcga/data_RNA_Seq_v2_expression_median_edited.csv", sep = ",", stringsAsFactors = FALSE, check.names = FALSE)
...
e <- subset(expressionDF, select=-c(`4`, variable))
...
grades <- factor(c("T0", "T1", "T1a", "T1b", "T2", "T2a", "T2b", "T3", "T3a", "T3b", "T4", "T4a", "T4b"))
grades <- ordered(grades, levels = c("T0", "T1", "T1a", "T1b", "T2", "T2a", "T2b", "T3", "T3a", "T3b", "T4", "T4a", "T4b"))
spec.stages <- factor(c("I", "IA", "IB", "II", "IIA", "IIB", "IIC", "III", "IIIA", "IIIB", "IIIC", "IV"))
spec.stages <- ordered(spec.stages, levels = c("I", "IA", "IB", "II", "IIA", "IIB", "IIC", "III", "IIIA", "IIIB", "IIIC", "IV"))
...
meta <- factor(c("M0", "M1", "M1a", "M1b", "M1c"))
meta <- ordered(meta, levels = c("M0", "M1", "M1a", "M1b", "M1c"))
