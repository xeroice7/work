...
setwd(dir = "Clay")
setwd(dir = "qRT")
file.path <- "/Users/PanopoulosLab1/Desktop/Clay/qRT/"
datadf <- data.frame(read.csv(paste(file.path, "072517CSC17genesJ2ZHCC_data.csv", sep=""), header = TRUE, sep = ",", stringsAsFactors = FALSE))
...
csc <- as.data.frame(filter(datadf, Population == "CSC"))
noncsc <- as.data.frame(filter(datadf, Population == "non-CSC"))
...
write.csv(cscDF, "Plate1072517MCF7CSCMSHits-Analyzed.csv")

#Setting up data to plot
plotcscDF <- cscDF[!duplicated(cscDF[,c("Target", "Population")]),]

positions <- c("non-CSC", "CSC")
